...
from utilities import index_of_closest, eigenvalues, drift, fbm_path, G, G0, G1, G2Mu, N_func
...
constant = integrate.quad(lambda t: G(m, t, mu, nu, H, N), 0.01, np.inf)[0]
values_analytical = [G(m, t, mu, nu, H, N) / constant for t in np.linspace(0.01, 1, num=N)]
...
check2 = [2. * epsilon * G1(m, t) / G0(m, t) for t in np.linspace(0.01, 1, num=N)]
...
plt.show()
#
#    values_simulated = (results[:, index_of_closest(m, x)] + 1)
#    values_simulated = [4096. if x==4097. else x for x in values_simulated]
#    values_simulated = np.array(values_simulated) / float(N)